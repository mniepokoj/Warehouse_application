...
class AddStoredProductComponent extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedProduct: '',
  ...
  handleSubmit = (event) => {
    ...
    const { selectedProduct: selectedStoredProduct, quantity } = this.state;
    this.handleSaveStoredProduct({ productId: selectedStoredProduct, quantity });
  ...
  handleSaveStoredProduct = async (newProductData) => {
    try {
      const response = await fetch('http://localhost:8086/ZTI_project-1.0-SNAPSHOT/api/warehouse/addStoredProduct', {
        method: 'PUT',
        headers: {
  ...
  render() {
    ...
    return (
      <div>
        {!isFormOpen && (
          <button onClick={this.handleOpenForm}>Dodaj produkt do magazynu</button>
        )}
...
export default AddStoredProductComponent;